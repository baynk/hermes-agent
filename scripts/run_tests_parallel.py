...
def _print_progress(
    ...
    Format:
      [done_tests/total_tests  p%  ✓files_passed ✗files_failed ⏱files_in_flight] status path (N✓ M✗ Xs)
    ...
    fw = len(str(files_passed + files_failed))
    ...
    msg = (
        ...
        f"✓{files_passed:>{fw}}|✗{files_failed:>{fw}}] "
...
def main() -> int:
    ...
    print(f"  Files: {len(files) - len(failures)} passed, {len(failures)} failed")
